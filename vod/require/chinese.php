...
define('_EncodePath', PHPVOD_ROOT . 'require/encode/');
...
class Chinese
{
	...
	var $IndexPoint = array('GBKtoUTF8'=>0, 'GBKtoUNICODE'=>0, 'UTF8toGBK'=>512, 'BIG5toUTF8'=>1024, 'BIG5toUNICODE'=>1024, 'UTF8toBIG5'=>1536, 'CHSStoCHST'=>2048, 'CHSTtoCHSS'=>2560);
	...
	function initConvert($SourceLang, $TargetLang, $ForceTable)
	{
		if(($SourceLang = $this->_getCharset($SourceLang)) && ($TargetLang = $this->_getCharset($TargetLang)) && $SourceLang != $TargetLang)
		{
			...
			$this->IconvEnabled || is_resource($this->TableHandle) || $this->TableHandle = fopen(_EncodePath . 'encode.table', "r");
		}
	}
	...
	function Convert($srcText, $SourceLang = '', $TargetLang = '', $ForceTable = false)
	{
		...
		switch($this->EncodeLang)
		{
			...
			case 'CHSStoCHST':
			case 'CHSTtoCHSS':
				return $this->_CHSConvertST($srcText);
				break;
			case 'GBKtoBIG5':
			case 'BIG5toGBK':
	...
	function CHSConvert($srcText, $SourceLang = 'GBK')
	{
		...
		{
			$handle = fopen(_EncodePath . 'gb-big5.table', "r");
		}
		else
		{
			$handle = fopen(_EncodePath . 'big5-gb.table', "r");
		}
		$encode = array();
