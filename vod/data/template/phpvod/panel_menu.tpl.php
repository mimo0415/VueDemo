...
echo '</li>';							
        ...
        <ul>
            <li><a href="<?php echo $db_bfn;?>">首页</a></li>
            <?php $cdb = pv_loop('class',"cid=0"); if(is_array($cdb)) { foreach($cdb as $c) { ?>            <li><a href="category.php?cid=<?php echo $c['cid'];?>" rel="submenu<?php echo $c['cid'];?>"><?php echo $c['caption'];?></a></li>
            <?php $mainarray[]=$c['cid']; ?>            <?php } } ?>            <?php if(is_array($hackdb['2'])) { foreach($hackdb['2'] as $h) { ?>            <li><a href="hack.php?hackname=<?php echo $h['directory'];?>"><?php echo $h['name'];?></a></li>
            <?php } } ?>        </ul>
    </div>
    ...
    <?php $subarray=class2array($_class); ?>    <?php if(is_array($subarray)) { foreach($subarray as $k => $v) { ?>    <?php if(!in_array($k, $mainarray)) continue; ?>    <ul id="submenu<?php echo $k;?>" class="ddsubmenustyle">
        <?php loopclass($v,$_class); ?>    </ul>
    <?php } } ?>