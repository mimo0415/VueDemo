<?php include gettpl('panel_flash'); $adcode = ad('nav'); if($adcode!='') { ?>
...
<div class="g-full f-mt10 f-fl">
    <div class="g-main f-fl">
        <div class="m-box">
            ...
            <div class="content">
                <ul class="m-list1">
                    <?php $videodb = pv_loop('video',"best=1|limit=10|order=lastdate DESC,postdate DESC,v.vid DESC"); if(is_array($videodb)) { foreach($videodb as $video) { ?>                    <li>
                        <div class="pic">
...
</div><?php $adcode = ad('index',1); if($adcode!='') { ?>
<div class="u-adindex f-mt15"><?php echo $adcode;?></div>
<?php } $adi=2; $c1db = pv_loop('class',"cid=0"); if(is_array($c1db)) { foreach($c1db as $c1) { ?><div class="g-full f-mt15 f-fl">
    <div class="g-main f-fl">
        <div class="m-box">
            <div class="title">
                <span class="f-fr f-fs12"><?php $c2db = pv_loop('class',"cid=$c1[cid]"); if(is_array($c2db)) { foreach($c2db as $c2) { ?>&nbsp;&nbsp;<a href="class.php?cid=<?php echo $c2['cid'];?>"><?php echo $c2['caption'];?></a><?php } } ?></span> <?php echo $c1['caption'];?>
            </div>
            <div class="content">
                <ul class="m-list1">
                    <?php $videodb = pv_loop('video',"cid=$c1[cid]|showsub=1|order=lastdate DESC,postdate DESC,v.vid DESC|limit=10"); if(is_array($videodb)) { foreach($videodb as $video) { ?>                    <li>
                        <div class="pic">
                            <a href="read.php?vid=<?php echo $video['vid'];?>" title="<?php echo $video['subject'];?>"><img src="<?php echo $video['picurl'];?>" alt="<?php echo $video['subject'];?>" /></a>
                        </div>
                        <div class="subject"><a href="read.php?vid=<?php echo $video['vid'];?>" title="<?php echo $video['subject'];?>"><?php echo $video['subject'];?></a></div>
    ...
    <div class="g-side 5 f-fr">
        <div class="m-box">
            ...
            <?php $rankarr = array('1'=>'yesterday_hits','2'=>'week_hits','3'=>'month_hits'); ?>            <?php if(is_array($rankarr)) { foreach($rankarr as $k => $t) { ?>            <?php $stylestr = $t=='yesterday_hits' ? 'block' : 'none'; ?>            <div class="content" id="c<?php echo $c1['cid'];?>_<?php echo $k;?>" style="display: <?php echo $stylestr;?>">
                <ul class="m-list2">
                    <?php $i=1; ?>                    <?php $videodb = pv_loop('video',"cid=$c1[cid]|showsub=1|order=$t DESC,hits DESC,v.vid DESC|limit=10|cachetime=1d"); if(is_array($videodb)) { foreach($videodb as $video) { ?>                    <?php if($i==1) { ?>
                    <li class="l1">
                        <a class="imgbg1" href="read.php?vid=<?php echo $video['vid'];?>" title="<?php echo $video['subject'];?>"><img src="<?php echo $video['picurl'];?>" alt="<?php echo $video['subject'];?>" /><span class="num1">1</span></a>
                        <p class="subject"><a href="read.php?vid=<?php echo $video['vid'];?>" title="<?php echo $video['subject'];?>"><?php echo $video['subject'];?></a></p>
                        <p>类别: <?php echo $video['class_name'];?></p>
...
</div><?php $adcode = ad('index',$adi); $adi++; if($adcode!='') { ?>
<div class="u-adindex f-mt15"><?php echo $adcode;?></div>
<?php } } } if($db_indexlink=='1') { ?>
<div class="g-full f-mt15 f-fl">
    <div class="m-box">
        <div class="title">友情链接</div>
        <div class="content f-p10">
            <?php if($db_indexmqlink=='1') { ?>
            <marquee scrolldelay="100" scrollamount="4" onmouseout="this.start();" onmouseover="this.stop();" behavior="alternate"><?php echo $_sharelink;?></marquee>
            <?php } else { ?>
            <?php echo $_sharelink;?>
            <?php } ?>
        </div>
