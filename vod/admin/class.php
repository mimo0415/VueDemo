...
elseif($action == 'edit_board')
{
	...
	if($step == 2)
	{
		...
		initvar(array('caption','link','tplfile'),'P');
		$basename = "$admin_file?adminjob=class&action=edit_board&cid=$cid";
		$db->update("UPDATE pv_class SET vieworder='$vieworder',caption='$caption',link='$link',tplfile='$tplfile' WHERE cid='$cid'");
		updatecache_class();
		adminmsg('operate_success');
	}
	else 
...
elseif($action == 'edit_sub')
{
	...
	if($step != 2)
	{
		...
		foreach($lsarray as $value)
		{
			...
			${$value} = "<table cellspacing='0' cellpadding='0' border='0' width='100%' align='center'><tr>";
			foreach($ltitle as $key => $val) //循环缺省和游客之外的所有用户组
			{
				...
				${$value} .= "<td><input type='checkbox' name='{$value}[]' value='$key'{$cflag}> $val</td>$htm_tr";
			}
			${$value} .= "</tr></table>";			
		}
	}
	else
	{
		...
		initvar(array('caption','type','orderway','link','tplfile','read_tplfile','play_tplfile','password'),'P');
