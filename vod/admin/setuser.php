...
elseif($action == 'editgroup')
{
	...
	if(!empty($selid)) del_user($selid, (boolean)$delmsg);
	adminmsg('operate_success');
}
elseif($action == 'edit')
{
	...
	{
		initvar(array('uid','groupid','gender','publicmail','receivemail','postnum','rvrc','money'), 'P', 2);
		initvar(array('username','password','check_pwd','email','regdate','icon','year','month','day','honor','signature','oicq','msn','site','userip'), 'P');
		...
		$bday = $year . "-" . $month . "-" . $day;
		$db->update("UPDATE pv_members SET groupid='$groupid',gender='$gender',regdate='$regdate',publicmail='$publicmail',receivemail='$receivemail',icon='$icon',bday='$bday',honor='$honor',signature='$signature',oicq='$oicq',msn='$msn',site='$site' WHERE uid='$uid'");
