...
function updatecache_gp($group)
{
	...
	if($group['gptype'] == 'system')
	{
		...
		$group['SYSTEM']['allowadminviewhide'] = $group['allowadminviewhide'];
		$group['SYSTEM']['permissions'] = $group['permissions'];
	}
	unset($group['allowadmincp'], $group['allowadminedit'], $group['allowadmindel'], $group['allowadminshow'], $group['allowadminviewhide'], $group['permissions']);
...
function updatecache_template($EXT = 'htm')
{
	...
	$fp = @opendir(PHPVOD_ROOT . "template/$tplpath/");
	if($fp)
	{
 ...
 * 更新模板数据缓存(tag cache)
 */
function updatecache_template_data()
{
	$pathname = PHPVOD_ROOT . 'data/cache/video';
	if(is_dir($pathname))
	{
 ...
 * 更新各栏目下的视频数
 */
function updatecache_video_count()
{
	global $db;
	...
	{
		$_video_count[$key] = 0; //初始化数据
		$_class_level[$value['lv']][] = $key; //按lv分组
	}

	krsort($_class_level); //按key逆序排列

	$query = $db->query("SELECT cid,COUNT(vid) AS num FROM pv_video GROUP BY cid");
	while($row = $db->fetch_array($query))
	{
		$_video_count[$row['cid']] = (int)$row['num'];
	}

	foreach($_class_level as $lv => $cidarr)
	{
		if($lv == '0') continue;
		foreach ($cidarr as $cid)
		{
			$cup = $_class[$cid]['cup'];
			$_video_count[$cup] += $_video_count[$cid];
		}
	}

	$_video_count = '$videocount = ' . pv_var_export($_video_count) . ';';
	writeover(PHPVOD_ROOT . 'data/cache/videocount.php', "<?php\r\n" . $_video_count . "\r\n?>");
}
?>