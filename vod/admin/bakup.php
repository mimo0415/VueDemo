...
elseif($admintype == 'bakin')
{
	...
	if(!$step)
	{
		...
		{
			if((!$dbpre || eregi("^pv_", $file) || eregi("^$dbpre", $file)) && eregi("\.sql$", $file))
			{
				$strlen = eregi("^$dbpre", $file) ? 15 + strlen($dbpre) : 18;
	...
	elseif($step == '2')
	{
		...
		if(!$count)
		{
			...
			while($file = readdir($handle))
			{
				if(eregi("^$pre", $file) && eregi("\.sql$", $file))	$count++;
			}
			$database_manager = array($admin['username'],$admin['password'],$admin['uid'],$admin['ucuid'],$admin['groupid']);
